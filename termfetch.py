...
STATUS_BAR_MESSAGE = (
    "press Q to exit | escape to dismiss popups | developer : {} | version : {}".format(
        AUTHOR, VERSION
    )
)
#######################
...
class Disk:
    def capacity() -> float:
        ...
        try:
            ch = Popen(
                ["wmic", "logicaldisk", "get", "size"], stdout=PIPE, stderr=STDOUT
            )
    ...
    def free() -> float:
        ...
        try:
            ch = Popen(
                ["wmic", "logicaldisk", "get", "freespace"], stdout=PIPE, stderr=STDOUT
            )
        except CalledProcessError:
    ...
    def ip4() -> str:
        ...
        try:
            ...
            try:
                ...
                root.show_error_popup(
                    "error occurred", f'could not get external ip4 due to error: "{e}"'
                )
                ip4 = "not detectable"
        finally:
            return ip4
    ...
    def ip6() -> str:
        ...
        try:
            ...
            try:
                ...
                root.show_error_popup(
                    "error occurred", f'could not get external ip6 due to error: "{e}"'
                )
                ip6 = "not detectable"
        finally:
            return ip6
...
def main():
    ...
    root.set_status_bar_text(STATUS_BAR_MESSAGE)
